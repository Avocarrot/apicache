...
function ApiCache() {
  ...
  this.middleware = function cache(duration, middlewareToggle) {
    ...
    return function cache(req, res, next) {
      var cached;
      var bypass = (req.headers['x-apicache-bypass'] || req.headers['cache-control'] == 'no-cache');
      if (!globalOptions.enabled || bypass || (_.isFunction(middlewareToggle) && !middlewareToggle(req, res))) {
        if (globalOptions.debug && bypass) {
